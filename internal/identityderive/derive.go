// Package identityderive 为一次已落库的采集运行推导 Pod 身份区间，
// 并单独记下这次推导的成败。
//
// 单独成包，是因为它有两个消费方：PULL 模式的采集器在落库之后自己推导，
// 而 PUSH 模式下这件事只能发生在平台侧 —— agent 读不到整张区间表
// （design doc 2026-08-18）。抄两份的那一天，只会是其中一处改了互斥的
// 取法而另一处没有，而两处的分歧不报错，只让区间被关在错的时刻。
...
// SaveTimeout 是记账那一步的上限。
//
// 与推导本身的上限分开：超时或取消之后，「这一轮的身份到底推出来没有」正是
// 最该留下来的东西，而复用那个已经取消的 ctx 会让它随之消失。
const SaveTimeout = 30 * time.Second

// Store 是一次身份推导要的落库口与串行化口。*snapshotstore.Store 满足它。
//
// 与采集的落库口分成两个接口，理由与它们分开同源（collection-layer spec §2.2）：
...
// Once 为一次已落库的采集运行推导身份区间，并单独记下这次推导的成败。
//
...
func Once(
	...
	if deriveErr != nil {
		...
		run.ErrorReason = errorReason(deriveErr)
	}
...
func recordDeriveRun(
	ctx context.Context, store Store, run snapshotstore.DeriveRun, logger *slog.Logger,
) {
...
func saveDeriveRun(ctx context.Context, store Store, run snapshotstore.DeriveRun) error {
