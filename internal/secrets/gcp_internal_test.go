...
// 只有 NotFound 是 ErrNotFound。
func TestNotFoundMapsToErrNotFound(t *testing.T) {
	err := status.Error(codes.NotFound, "secret projects/p/secrets/x not found")
	if got := classifyAccessError(err); !errors.Is(got, ErrNotFound) {
		t.Fatalf("classifyAccessError(NotFound) = %v, want ErrNotFound", got)
	}
}

// 其余状态码都不是 ErrNotFound。
//
...
func TestNonNotFoundStatusesAreNotErrNotFound(t *testing.T) {
	for _, code := range []codes.Code{
		...
		if errors.Is(got, ErrNotFound) {
			t.Errorf("classifyAccessError(%v) = ErrNotFound, want it kept distinct from a missing secret", code)
		}
	}
}

// 非状态错误（例如上下文取消、传输层错误）也不是 ErrNotFound。
func TestPlainErrorsAreNotErrNotFound(t *testing.T) {
	got := classifyAccessError(errors.New("dial tcp: i/o timeout"))
	if errors.Is(got, ErrNotFound) {
		t.Fatal("classifyAccessError(plain error) = ErrNotFound, want a lookup failure")
	}
}
...
func TestWrappedNotFoundIsStillRecognised(t *testing.T) {
	...
	if got := classifyAccessError(wrapped); !errors.Is(got, ErrNotFound) {
		t.Fatalf("classifyAccessError(wrapped NotFound) = %v, want ErrNotFound", got)
	}
}
...
func TestAPIErrorNotFoundIsRecognised(t *testing.T) {
	...
	if got := classifyAccessError(ae); !errors.Is(got, ErrNotFound) {
		t.Fatalf("classifyAccessError(*apierror.APIError NotFound) = %v, want ErrNotFound", got)
	}
	perm, ok := apierror.FromError(status.Error(codes.PermissionDenied, "denied"))
	if !ok {
		t.Fatal("apierror.FromError() = !ok, want a status error to convert")
	}
	if got := classifyAccessError(perm); errors.Is(got, ErrNotFound) {
		t.Fatal("classifyAccessError(*apierror.APIError PermissionDenied) = ErrNotFound, want a lookup failure")
	}
}
