// Package gcpsecrets 是 secrets.Resolver 的 Secret Manager 实现。
//
// 单独成包而不是留在 internal/secrets 里，是为了让 internal/secrets 保持
// 叶子：它被 internal/registry 引用（复用 ValidateRef，两份字符类定义迟早
// 会走样），而 internal/registry 是 store、policygen 与 HTTP 层共同依赖的
// 领域类型包。把云 SDK 留在同一个包里，等于让 Secret Manager、gRPC、OTel
// 与 genproto 出现在全平台几乎每一条依赖链上 —— 而 HANDOFF.md 里那条查
// 纯净度的命令只看直接 import，它看到的只有 internal/secrets，看不见这层。
// 围栏破在检查照不到的地方，比破在检查得到的地方更值得挪一次包。
package gcpsecrets
...
// 字符集已被 secrets.ValidateRef 收窄，因此他无法表达出配置划定的这个
// 前缀之外的任何资源路径。越权不是被检查出来的，是拼不出来的。
//
...
// Resolver 从 Google Secret Manager 解析凭据，是生产实现。
//
...
func NewResolver(ctx context.Context, project, prefix string) (*Resolver, error) {
	...
	return &Resolver{client: client, project: project, prefix: prefix}, nil
}
...
//
// 开头这道 ValidateRef 不是重复校验：绑定保存时确实也校验过，但库里可能
// 存着校验规则收紧之前写进去的引用，而拼资源路径这一步就在下面三行 ——
// 它是「越权路径拼不出来」这条性质在运行时的落点，不是入口处的礼貌检查。
// 「Resolve 是否仍然调用它」由 resolver_internal_test.go 里那条断言证明
// （用 nil client 构造，因此一旦这道校验被删，测试会在够到客户端时就炸）。
...
//
// 只有 NotFound 映射成 secrets.ErrNotFound：引用取不到是平台侧配置问题，
// 取到了但认证失败是仓库侧权限问题，两者的处置人不同（见 secrets/resolver.go
// 与 §2.4）。PermissionDenied 不算 NotFound —— 服务账号没有访问权同样是
// 平台侧问题，但它不是「不存在」，混成一类会把「权限没开」说成「引用写错了」。
//
...
// 解包。这是依赖的行为不是本包的，所以 resolver_internal_test.go 里对「包了
// 一层的 NotFound」单独有一条断言 —— 哪天升级把解包去掉了，那条会红，而不是
