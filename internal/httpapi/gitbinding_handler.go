...
//
// 两个字段就是绑定的全部可写内容（design doc 2026-08-13 §4）。它们的
// 共同点是：都由操作者提供，平台自己产生不了。凡是平台自己产生的东西，
// 都不在这里。
//
// 仓库地址、分支与凭据引用**不在这里**：它们属于仓库，从这次拆分起由
// /git-repos 那组端点管（design doc §3.2）。留在这里就是两个真相来源 ——
// 操作者在集群页改了地址，仓库页显示的还是旧的，而平台真正会去连的是
// 哪一个，只能靠读代码才知道。
//
...
// 「可以下发」。它们由本 handler 从 verifyBindingPath 的返回值填入。
//
...
// handleBindGitRepo 把一个集群绑定到一个已存在的策略仓库。
//
// PUT 而非 PATCH：两个字段是绑定的全部可写内容，这里写整行。挂成 PATCH
...
func handleBindGitRepo(d Deps) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		b := p.toBinding()
		// 集群不存在先于出站问一次：一次针对不存在集群的 SSH 握手救不了
		...
		// 领域校验与仓库读取先于出站：BindGitRepo 会自己再校验一次，那才是
		// 不可绕过的关卡；这一道是**时序**上的。填了一半的绑定、或指向一个
		// 存不进库的仓库，本来就保存不成，却要先花掉一次带秒级超时的握手，
		// 操作者才等到一句和超时无关的报错。
		repo, ok := bindingVerificationTarget(w, r, d, b)
		if !ok {
			return
		}
		...
		//
		// 仓库级先于路径级，且这一步的结论不落到仓库行上：理由与
		// handleVerifyGitBinding 那处相同，写在那里。
		repoResult, _ := verifyRepo(r.Context(), d, repo)
		b.VerifyResult, b.VerifiedAt = verifyBindingPath(r.Context(), d, repo, repoResult, b.PolicyPath)
		...
		response.WriteOK(w, bindingVerifyStatus{VerifyResult: b.VerifyResult, VerifiedAt: b.VerifiedAt})
	}
}

// handleUnbindGitRepo 解除一个集群的 Git 绑定。
//
// 解绑是自己的动词，不是「两个字段全空的一次保存」：后者要靠调用方与
